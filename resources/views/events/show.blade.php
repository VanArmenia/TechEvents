...
@section('content')
    ...
    <div class="row event-bar ">
        <div class="col-md-8 hidden-sm hidden-xs text-muted">
            ...
            / <a href="{{ route('states.show', ['id' => $event->state->abbreviation]) }}">{{$event->state->name}}</a>
            / {{ $event->city }}
            / {{ $event->name }}
            / {{ $event->max_attendees }}
            ...
            <p class="pull-right">
                ...
                @if (Auth::check())
                    <favorite event-id="{{ $event->id }}"
                              favorited="{{ Auth::user()->favoritedEvent($event->id) }}">
    ...
    <div class="row ">
        ...
        <div class="col-md-9">
            ...
            @forelse (App\Event::upcoming()->where('id', '!=', $event->id)->nearby($event->lat, $event->lng, 500)->get()->take(5) as $nearbyEvent)
                <p>
                    <a href="{{ route('events.show', ['id' => $nearbyEvent]) }}">{{$nearbyEvent->name}}</a>
                    <b></b> ({{ round($nearbyEvent->distance, 2) }} miles)<br />
                    ...
                    @elseif (Auth::check() and Auth::user()->isOrganizer($event))
                        ...
                        <div class="event-ticket-box">
                            <p style="text-align: center;">
                                ...
                                <a href="{{ route('events.edit', ['event' => $event]) }}">Edit Event Details</a>
